<template>
  <div class="common-goods-detail">
    ...
    <div class="goods-info">
      ...
      <div class="exchange"><img class="hot" src="../../../assets/img/welfare/icon-colleague.png" alt="" />{{ goodsInfo.exchangeCnt }}位同事曾兑换</div>
    </div>
    ...
    <div class="fixed-btn-wrap">
      <div>
        福利资格：<span class="stress">礼品任选*{{ goodsInfo.unExchangeCnt }}</span>
      </div>
      <exchange-btn class-name="exchange-btn" :disabled="loading" :goods-info="goodsInfo" @submit="confirm"> </exchange-btn>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    virtual() {
      const { carWashTicket, videoMember, viewingTicket, starbucks, oilCard } = sysConfig
      const itemCatId = this.goodsInfo.itemCatId
      return itemCatId === carWashTicket || itemCatId === videoMember || itemCatId === viewingTicket || itemCatId === starbucks || itemCatId === oilCard
    },
    tipImg() {
      const imgName = this.virtual ? 'virtual-goods-tip' : 'real-goods-tip'
  ...
  methods: {
    ...
    async exchangeGoods() {
      try {
        this.loading = true
        const res = await this.$WelfareCustOrder.welfareExchange(this.activityId, this.goodsInfo.goodsId)
