...
export default {
  ...
  methods: {
    getQueryParams() {
      const { idNumber, phone } = this.$route.query
      if (idNumber && phone) {
    ...
    async joinActivity() {
      ...
      const res = await this.$Elife.xjoin(this.idNumber, this.phone, this.code, this.codeInfo.codeId)
      this.postElifeX(res.data, false)
    },
    async sendCode() {
      helper.toast('验证码已发送，请注意查收')
      const res = await this.$Inside.sendCode(this.phone, '0')
