<template>
  <div class="t-finance-order-page t-finance-page">
    <template v-if="!orderEnd">
      ...
      <activity-rule
        :order-start="productInfo.intentStartDate"
        :order-end="productInfo.intentEndDate"
        :buy-start="productInfo.subscribeStartDate"
        :buy-end="productInfo.subscribeEndDate"
      ></activity-rule>
      ...
      <div class="bottom-action">
        <div class="fixed-time">
          <template v-if="orderStart"> 剩余<count-down :now="productInfo.nowDate" :target="productInfo.intentEndDate" @end="orderEnd = true"></count-down>结束 </template>
          <template v-else> 距离<count-down :now="productInfo.nowDate" :target="productInfo.intentStartDate" @end="toStart"></count-down>开始 </template>
        </div>
