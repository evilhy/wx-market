    ...
    <van-swipe ref="swipe" v-if="showSwipe" class="my-swipe" vertical :show-indicators="false" :loop="false">
      <van-swipe-item>
        <div class="p7 wrap">
          ...
          <div class="next"><img src="../../assets/img/yearBill/next.png" alt="" /><span>向上滑动翻页</span></div>
        </div>
      </van-swipe-item>
      <van-swipe-item>
        <div class="p8 wrap">
        ...
        <div class="p3 wrap">
          ...
          <div class="p3-d1">
            <div class="cell" v-for="(item, index) in yearBill.fundWages" :key="index">
              ...
              <van-progress
                ref="progress"
                :percentage="item.fundPercent"
                track-color="rgba(205,143,70,0.20)"
                color="linear-gradient(90deg, #FF7C4C 0%, #EC3616 100%)"
                stroke-width="10"
                :show-pivot="false"
              />
            </div>
...
export default {
  ...
  computed: {
    monthCountDesc() {
      const { monthCount } = this.yearBill
      if (monthCount <= 3) {
        return '以后五块钱以上的活动不叫你！'
      }
      if (monthCount > 3 && monthCount <= 7) {
        return '啥时候能实现车厘子自由！'
      }
      if (monthCount > 7 && monthCount <= 10) {
        return '兄die，你是真的稳！'
      }
      return '你大爷永远是你大爷！'
    },
    percent() {
      const { percent } = this.yearBill
      if (percent < 10) {
  ...
  methods: {
    ...
    async getYearBill() {
      const res = await this.$Bill.bill()
