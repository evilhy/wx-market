  ...
  <div class="page withdrawal-confirm-page" :class="{ white: step === 2 }">
    <template v-if="step === 1">
      ...
      <div class="row-flex-wrap">
        ...
        <div class="row-flex row-flex--between">
          <span class="label">资金月份</span><span class="value">{{ wageDetail.fundDateVal }}</span>
        </div>
        <div class="row-flex row-flex--between">
          <span class="label">资金类型</span><span class="value">{{ wageDetail.fundTypeVal }}</span>
        </div>
        <div class="row-flex row-flex--between">
          <span class="label">发放机构</span><span class="value">{{ wageDetail.groupName }}</span>
        </div>
      </div>
    ...
    <van-action-sheet v-if="signInfo.signStatusVal" class="tax-signup-action-sheet" close-on-popstate v-model:show="actionSheetShow" title="完成以下任务就可以提现啦！">
      <div class="signup-item">
        ...
        <van-button v-else plain size="small" :color="attestBtn.color" @click="$router.push({ name: 'uploadIdentity', params: { withdrawalLedgerId: wageDetail.withdrawalLedgerId } })">{{
          attestBtn.text
        }}</van-button>
      </div>
      <div class="signup-item">
...
export default {
  ...
  methods: {
    ...
    async getSignDetail() {
      const res = await this.$Tax.signingDetails(this.wageDetail.withdrawalLedgerId)
      this.signInfo = res.data
    },
    transBanks(list = []) {
      return list.map((item) => {
        const { issuerName, cardNo, cardTypeVal } = item
        return {
          ...item,
