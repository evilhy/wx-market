  ...
  <div class="page withdrawal-detail-page">
    ...
    <div class="row-flex-wrap">
      <div class="row-flex row-flex--between">
        <span class="label">方案名称</span><span class="value">{{ wageDetail.wageSheetName }}</span>
      </div>
      <div class="row-flex row-flex--between">
        <span class="label">资金类型</span><span class="value">{{ wageDetail.fundTypeVal }}</span>
      </div>
      <div class="row-flex row-flex--between">
        <span class="label">付款方</span><span class="value">{{ wageDetail.accountName }}</span>
      </div>
      <div class="row-flex row-flex--between">
        <span class="label">付款账号</span><span class="value">{{ wageDetail.accountOpenBank }}（尾号{{ wageDetail.account.slice(-4) }}）</span>
      </div>
      <div class="row-flex row-flex--between">
        <span class="label">收款方</span><span class="value">{{ wageDetail.custName }}</span>
      </div>
      <div class="row-flex row-flex--between">
      ...
      <div class="row-flex row-flex--between">
        <span class="label">备注</span><span class="value">{{ wageDetail.remark }}</span>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      wageDescList: [
        // 0:待提现、1:提现成功、2:提现失败、3:处理中 4:超时未提现
        {
          className: 'gray-text',
          text: '钱包收款'
        },
  ...
  computed: {
    showWithdraw() {
      // 是否启用员工提现（0.停用 1.启用）
      const { withdrawalStatus, withdrawStatus } = this.wageDetail
      return (withdrawalStatus === 0 || withdrawalStatus === 2) && withdrawStatus === 1
    },
    canWithdraw() {
      // 付款账户状态(0:正常、1:异常)
      const { accountStatus, transAmount } = this.wageDetail
      return accountStatus === 0 && transAmount > 0
    },
    btnText() {
      const { withdrawalStatus } = this.wageDetail
  ...
  methods: {
    async getDetail() {
      try {
        this.loading = true
        const res = await this.$Wallet.withdrawalRecordDetail(this.id)
        const { withdrawalRecordLog = {}, withdrawalLedgerDetail = {} } = res.data
