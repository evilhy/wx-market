<template>
  <div class="balance-item">
    <div class="balance-item__title" @click="toPage('withdrawalDetail')">
...
export default {
  ...
  computed: {
    showWithdraw() {
      ...
      const { withdrawalStatus, withdrawStatus } = this.item
      return (withdrawalStatus === 0 || withdrawalStatus === 2 || withdrawalStatus === 4) && withdrawStatus === 1
    },
    canWithdraw() {
      // accountStatus: 0账户正常 1账户异常
      const { accountStatus, transAmount, withdrawalStatus } = this.item
