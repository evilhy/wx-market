...
export default class HttpEngine {
  ...
  [adapterHandler](config) {
    ...
    return new Promise((resolve, reject) => {
      let mock = new MockerEngine(status, this[$mockTimeout])
      mock.url = config.baseURL + config.url
  ...
  [printResponseInfo](response) {
    ...
    debug.url = config.baseURL + config.url
    debug.method = config.method
    debug.headers = config.headers
    debug.query = config.params
    debug.body = config.data
    debug.status = status
    debug.response = data
    debug.print()
  }
  ...
  [printResponseError](error) {
    ...
    let { config, response, message } = error
    debug.url = config.url
    debug.method = config.method
    debug.headers = config.headers
    debug.query = config.params
    debug.body = config.data
    debug.status = (response && response.status) || 0
    debug.response = (response && response.data) || undefined
