    ...
    <div class="pwd-keyboard-container" :class="type === 'complete' ? `complete-${completeImgType}` : ''">
      <div class="key-container">
        ...
        <template v-if="type === 'complete'">
          <div class="key-item" v-for="(item, index) in dataSource[type][completeImgType]" :key="index" :style="calKeyStyle(index)" @touchstart="dealEvent(item)"></div>
        </template>
        <!--  数字、身份证键盘 -->
        <template v-else>
          <div class="key-item" v-for="(item, index) in dataSource[type]" :key="index" :style="calKeyStyle(index)" @touchstart="dealEvent(item)"></div>
        </template>
      </div>
    </div>
  </van-popup>
...
export default {
  ...
  methods: {
    ...
    calKeyStyle(index) {
      const { type, completeImgType } = this
      if (type === 'complete' && completeImgType !== 'number') {
        if (index <= 18) {
          return `left:${((75 * (index % 10) + parseInt(index / 10, 10) * 37) / 750) * 100}vw;top:${parseInt(index / 10, 10) * 13.6}vw;width: 8.53vw;height: 10.533vw;`
        }
        if (index === 19 || index === 27) {
          return `left: ${parseInt(index / 20, 10) * 86.67}vw; top: 27.2vw; width: 11.867vw; height: 10.533vw;`
        }
        if (index <= 26) {
          return `left: ${((112 + (index % 20) * 75) / 750) * 100}vw; top: 27.2vw; width: 8.533vw;height: 10.533vw;`
        }
        if (index === 28 || index === 29) {
          return `left: ${parseInt(index / 29, 10) * 28.267}vw; top: 40.8vw; width: 26.8vw;height: 10.533vw;`
        }
